...
Model::Model(std::string const& path, bool gamma)   // 伽马校正也没用上
{
    loadModel(path);    // path = "MODELS/cornstick/Model.obj"
}

// 读取模型并存入 vector<Mesh>
void Model::loadModel(std::string const& path)  // path = "MODELS/cornstick/Model.obj"
{
    ...
    // directory = "MODELS/cornstick" 找到该模型所处目录
